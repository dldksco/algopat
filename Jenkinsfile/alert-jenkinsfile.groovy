pipeline {
    ...
    environment {
        BUILD_BRANCH = "${env.gitlabBranch == 'dev-back-alert'}"
    }
    stages {
        ...
        stage("Set environment") {
            ...
            steps {
                echo "Copy require file to alert folder"
                sh 'cp /var/jenkins_home/util/alert/docker-compose.yml .'
                sh 'cp /var/jenkins_home/util/alert/application.yml ./alert/src/main/resources/application.yml'
                sh 'cp /var/jenkins_home/util/alert/Dockerfile ./alert/'
            }
        }
        stage('Docker down') {
            when {
