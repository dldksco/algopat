pipeline {
    ...
    environment {
        BUILD_BRANCH = "${env.gitlabBranch == 'dev-back-auth'}"
    }
    stages {
        ...
        stage("Set environment") {
            ...
            steps {
                echo "Copy require file to auth folder"
                sh 'cp /var/jenkins_home/util/auth/docker-compose.yml .'
                sh 'cp /var/jenkins_home/util/auth/application.yml ./auth/src/main/resources/application.yml'
                sh 'cp /var/jenkins_home/util/auth/Dockerfile ./auth/'
            }
        }
        stage('Docker down') {
            when {
