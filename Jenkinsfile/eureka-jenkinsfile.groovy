pipeline {
    ...
    environment {
        BUILD_BRANCH = "${env.gitlabBranch == 'dev-back-eureka'}"
    }
    stages {
        ...
        stage("Set environment") {
            ...
            steps {
                echo "Copy require file to user folder"
                sh 'cp /var/jenkins_home/util/eureka/docker-compose.yml .'
                sh 'cp /var/jenkins_home/util/eureka/application.yml ./eureka/src/main/resources/application.yml'
                sh 'cp /var/jenkins_home/util/eureka/Dockerfile ./eureka/'
            }
        }
        stage('Docker down') {
            when {
