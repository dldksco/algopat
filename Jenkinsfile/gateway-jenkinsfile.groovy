pipeline {
    ...
    environment {
        BUILD_BRANCH = "${env.gitlabBranch == 'dev-back-api-gateway'}"
    }
    stages {
        ...
        stage("Set environment") {
            ...
            steps {
                echo "Copy require file to api-gateway folder"
                sh 'cp /var/jenkins_home/util/api-gateway/docker-compose.yml .'
                sh 'cp /var/jenkins_home/util/api-gateway/application.yml ./api-gateway/src/main/resources/application.yml'
                sh 'cp /var/jenkins_home/util/api-gateway/Dockerfile ./api-gateway/'
            }
        }
        stage('Docker down') {
            when {
