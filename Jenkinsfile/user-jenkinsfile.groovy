pipeline {
    ...
    environment {
        BUILD_BRANCH = "${env.gitlabBranch == 'dev-user'}"
    }
    stages {
        ...
        stage("Set environment") {
            ...
            steps {
                ...
                sh 'cp /var/jenkins_home/util/user/docker-compose.yml .'
                sh 'cp /var/jenkins_home/util/user/application.yml ./user/src/main/resources/application.yml'
                sh 'cp /var/jenkins_home/util/user/Dockerfile ./user/'
            }
        }
        stage('Docker down') {
            when {
