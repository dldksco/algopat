pipeline {
    ...
    environment {
        BUILD_BRANCH = "${env.gitlabBranch == 'dev-lang'}"
    }
    stages {
        ...
        stage("Set environment") {
            ...
            steps {
                echo "Copy require file to fastapi folder"
                sh 'cp /var/jenkins_home/util/fastapi/docker-compose.yml .'
                sh 'cp /var/jenkins_home/util/fastapi/Dockerfile ./fastapi'
                sh 'cp /var/jenkins_home/util/fastapi/.env ./fastapi'
            }
        }
        stage('Docker down') {
            when {
