pipeline {
    ...
    environment {
        BUILD_BRANCH = "${env.gitlabBranch == 'dev-front'}"
    }
    stages {
        ...
        stage("Set environment") {
            ...
            steps {
                ...
                sh 'cp /var/jenkins_home/util/frontend/docker-compose.yml .'
                sh 'cp /var/jenkins_home/util/frontend/nginx.conf ./frontend/'
                sh 'cp /var/jenkins_home/util/frontend/Dockerfile ./frontend/'
            }
        }
        stage('Docker down') {
            when {
