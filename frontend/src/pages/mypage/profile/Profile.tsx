...
export const Profile = () => {
  ...
  const { isLoading, error, data } = useQuery(
    ["tierupdate", authorized.isAuthorized, authorized.userId],
    async () => {
      const response = userCheck(authorized.userId);
      console.log("checkcheck");
      return response;
    },
    { enabled: authorized.isAuthorized }
  );
  ...
  const dummyData = {
    ...
    grass: [
      {
        problemList: [
          { problemId: 10, problemName: "배열돌리기12" },
          { problemId: 11, problemName: "배열돌리기12" },
          { problemId: 12, problemName: "배열돌리기12" },
        ],
      },
    ],
  };
  for (let i = 0; i < 365; i++) {
    const obj = {
      problemList: [
        { problemId: 40, problemName: "배열돌리기12" },
        { problemId: 41, problemName: "배열돌리기12" },
        { problemId: 42, problemName: "배열돌리기12" },
      ],
    };
          ...
          <div className={style.setting}>
            ...
            {modalOpen && (
              <Modal setModalOpen={setModalOpen}>
                <div className={style.modal} style={{ margin: "10px 20px" }}>
                  <h1>Solved AC 티어 연동하기</h1>
