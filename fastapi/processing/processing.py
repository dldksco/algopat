...
async def processing(data : ProblemData, send_callback):
    ...
    message_dto = MessageDTO("코드 요약 완료", "ok", user_seq)
    ...
    # @@@@@@@@@@@@@@@@@@ 이부분 수정 @@@@@@@@@@@@@@@@@@@@
    message_dto = MessageDTO("GPT 응답 생성 끝","ok", user_seq)
    await send_callback("alert", message_dto)
...
async def filtering_input_data(data : ProblemData, send_callback):
    ...
    if code_language == "unknown":
        message_dto = MessageDTO("인풋 데이터 필터링", "미지원 언어입니다.", data.userSeq)
        await send_callback("alert", message_dto)
        return True
    if data.resultCategory != "ac":
        message_dto = MessageDTO("인풋 데이터 필터링", "틀린 코드는 평가할 수 없습니다.", data.userSeq)
        await send_callback("alert", message_dto)
        return True