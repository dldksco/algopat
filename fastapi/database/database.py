...
async def save_problem_origin(problem_id : str, data : ProblemData):
    ...
    if await db_problem.check_problem_is_exist(problem_id, await get_session()) == False:
        logger.info("문제 DB 저장")
        await db_problem.insert_problem(data, await get_session())

async def save_problem_summary(problem_id : str, summary_json):
    logger.info("GPT 문제 요약 존재 여부 확인")
    if await db_problem.check_gpt_problem_summary_is_exist(problem_id, await get_session()) == False:
        logger.info("GPT 문제 요약 DB 저장")
        await db_problem.insert_gpt_problem_summary(summary_json, await get_session())

async def save_user_problem_origin(problem_id : str):
    logger.info("회원 푼 문제 불러오기")
    userSubmitProblemData = await db_problem.get_user_submit_problem(problem_id, await get_session())   
    if userSubmitProblemData is None:    
        ...
        userSubmitProblemData = await db_problem.insert_user_submit_problem(userSubmitProblemData, await get_session())
    return userSubmitProblemData.user_submit_problem_seq

async def check_gpt_problem_summary_is_exist(problem_id : str):
    return await db_problem.check_gpt_problem_summary_is_exist(problem_id, await get_session())

async def get_gpt_problem_summary(problem_id : str):
    return await db_problem.get_gpt_problem_summary(problem_id, await get_session())
...
async def save_user_submit_solution_origin(user_submit_problem_seq : int, problem_id : str, data : ProblemData):
    ...
    user_submit_solution_seq = await db_problem.insert_user_submit_solution(UserSubmitSolutionData, await get_session())
...
async def save_gpt_solution(user_submit_solution_seq, result):
    ...
    await db_problem.insert_gpt_solution(GPTSolutionData, await get_session())